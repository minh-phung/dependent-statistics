...
All three statistics listed above are computed, between $f(x)$ and $x$, for each element in the sets. Given that mutual information (in this case) is based on nearest-neighbour/have a hyperparameter, the chosen values are $k=1,..., 0.10 * n$, where $n = len(x)$, the number of $x$ points.
